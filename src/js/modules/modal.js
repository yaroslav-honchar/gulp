  ...
  constructor(options = {}) {
    if (options.modalSelector && options.triggerSelector) {
      ...
      this.disableScrollbar =
        options.disableScrollbar !== undefined
          ? options.disableScrollbar
          : true;
      this.closeOnLightbox =
        options.closeOnLightbox !== undefined ? options.closeOnLightbox : true;
      this.closeOnEsc =
      ...
      this.on =
        {
          beforeInit:
            options.on && options.on.beforeInit
              ? options.on.beforeInit.bind(this.on)
              : null,
          init:
            options.on && options.on.init
              ...
              : null,
          beforeOpen:
            options.on && options.on.beforeOpen
              ? options.on.beforeOpen.bind(this.on)
              : null,
          afterOpen:
            options.on && options.on.afterOpen
              ? options.on.afterOpen.bind(this.on)
              : null,
          beforeClose:
            options.on && options.on.beforeClose
              ? options.on.beforeClose.bind(this.on)
              : null,
          afterClose:
            options.on && options.on.afterClose
              ? options.on.afterClose.bind(this.on)
              : null,
        } || {};
  ...
  showModal = e => {
    this.modal.forEach(modal => {
      if (
        ...
        e.currentTarget.dataset.modalTarget.toLowerCase()
      ) {
